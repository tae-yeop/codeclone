...
class StableDiffusionReferencePipeline(StableDiffusionPipeline):
    ...
    def __call__(
        ...
        def hacked_mid_forward(self, *args, **kwargs):
            eps=1e-6
            # 원본 mid block의 forward를 한번 수행
            x = self.original_forward(*args, **kwargs)
            if MODE == 'write':
                # 만약 write이면 나온 결과에 대한 mean, var 저장해둠
            ...
            if MODE == 'read':
                ...
                if len(self.mean_bank) > 0 and len(self.var_bank) >0:
                    # 현재 이미지에 대한 mean, var 구함
                    vae, mean = torch.var_mean(x, dim=(2,3), keepdim=True,
                                               correction=0)
                    std = torch.maximum(var, torch.zeros_like(var) + eps) ** 0.5
                    mean_acc = sum(self.mean_bank) / float(len(self.mean_bank))
                    var_acc = sum(self.var_bank) / float(len(self.var_bank))
                    std_acc = torch.maximum(var_acc, torch.zeros_like(var_acc) + eps) ** 0.5
                    # bank에 있던 std와 mean으로 modulation
                    x_uc = (((x - mean)) * std_acc) + mean_acc
                    x_c = x_uc.clone()
                    if do_classifier_free_guidance and style_fidelity > 0:
                        x_c[uc_mask] = x[uc_mask]
                    x = style_fidelity * x_c + (1 - style_fidelity) * x_uc
